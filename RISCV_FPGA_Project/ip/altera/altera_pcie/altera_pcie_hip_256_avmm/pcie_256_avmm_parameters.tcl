...
proc add_pcie_hip_parameters_ui_system_settings {} {
   ...
   set_parameter_property gen123_lane_rate_mode_hwtcl ALLOWED_RANGES {"Gen2 (5.0 Gbps)" "Gen3 (8.0 Gbps)"}
   ...
   set_parameter_property gen123_lane_rate_mode_hwtcl DESCRIPTION "Selects the maximum data rate of the link.Gen3 (8.0 Gbps) and Gen2 (5.0 Gbps) are supported."
...
if { [ regexp x8 $lane_mask_hwtcl ] && [ regexp Gen3 $gen123_lane_rate_mode_hwtcl ] } {   ## Gen3x8
   ...
   } elseif { [ regexp x4 $lane_mask_hwtcl ] && [ regexp Gen3 $gen123_lane_rate_mode_hwtcl ] } { ## Gen3x4
    ...
    } elseif { [ regexp x8 $lane_mask_hwtcl ] && [ regexp Gen2 $gen123_lane_rate_mode_hwtcl ] } { ## Gen2x8
            send_message info "The application clock frequency (pld_clk) is 125 Mhz"
            set_parameter_value pld_clk_MHz 1250
            set_parameter_value millisecond_cycle_count_hwtcl 124250
  ...
  if {$use_atx_pll==1} {
      ...
      if { $fhrc == 1  } {
        ...
        set_parameter_value hip_hard_reset_hwtcl 0
      } elseif { [ regexp Gen1 $gen123_lane_rate_mode_hwtcl ] ||  [ regexp Gen2 $gen123_lane_rate_mode_hwtcl ] } {
        set_parameter_value hip_hard_reset_hwtcl 1
        send_message info "Hard Reset Controller enabled"
      } else {
        set_parameter_value hip_hard_reset_hwtcl 0
...
proc validation_parameter_bar {} {
   ...
   if { [ regexp endpoint $port_type_hwtcl ] } {
      if { $bar_type_hwtcl(0) == $DISABLE_BAR && $bar_type_hwtcl(1) == $DISABLE_BAR && $bar_type_hwtcl(2) == $DISABLE_BAR && $bar_type_hwtcl(3) == $DISABLE_BAR && $bar_type_hwtcl(4) == $DISABLE_BAR && $bar_type_hwtcl(5) == $DISABLE_BAR } {
         send_message error "This component requires to enable one BAR"
      }
