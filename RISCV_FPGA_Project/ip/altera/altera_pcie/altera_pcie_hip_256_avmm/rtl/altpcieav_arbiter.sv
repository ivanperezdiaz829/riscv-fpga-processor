 ...
 always_comb
  begin
    case(arb_state)
      ARB_IDLE :
       ...
       ARB_TXS_GRANT :
         if(DMARdArbReq_i & ~TxsArbReq_i)
          arb_nxt_state <= ARB_RD_GRANT;
        else if(DMAWrArbReq_i & ~TxsArbReq_i)
          arb_nxt_state <= ARB_WR_GRANT;
      ...
      ARB_RD_GRANT:
        if(DMAWrArbReq_i & ~DMARdArbReq_i)
          arb_nxt_state <= ARB_WR_GRANT;
        else if(RxmArbReq_i &  ~DMARdArbReq_i)
          arb_nxt_state <= ARB_RXM_GRANT;
      ...
      ARB_WR_GRANT:
        if(DMARdArbReq_i & ~DMAWrArbReq_i)  
            arb_nxt_state <= ARB_RD_GRANT;         
        else if(RxmArbReq_i & ~DMAWrArbReq_i)
          arb_nxt_state <= ARB_RXM_GRANT;
        else if(TxsArbReq_i & ~DMAWrArbReq_i)
          arb_nxt_state <= ARB_TXS_GRANT;
        else if(DMAWrArbReq_i)
          arb_nxt_state <= ARB_WR_GRANT;
        else
      ...
      ARB_RXM_GRANT:
        if(TxsArbReq_i & ~RxmArbReq_i)
          ...
          arb_nxt_state <= ARB_RD_GRANT;
        else if(DMAWrArbReq_i & ~RxmArbReq_i)
          arb_nxt_state <= ARB_WR_GRANT;
        else if(RxmArbReq_i)
          arb_nxt_state <= ARB_RXM_GRANT;
        else
...
/// Assign grant outputs
     ...
     assign DMAWrArbGrant_o = arb_state[3];  
