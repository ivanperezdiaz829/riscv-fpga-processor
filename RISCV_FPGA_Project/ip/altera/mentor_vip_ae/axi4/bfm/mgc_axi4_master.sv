...
interface mgc_axi4_master #(int AXI4_ADDRESS_WIDTH = 64, int AXI4_RDATA_WIDTH = 1024, int AXI4_WDATA_WIDTH = 1024, int AXI4_ID_WIDTH = 18, int AXI4_USER_WIDTH = 8, int AXI4_REGION_MAP_SIZE = 16, int index = 0)
(
    input  ACLK,
    input  ARESETn,
    output AWVALID,
...
`endif    // _MGC_VIP_VHDL_INTERFACE
    ...
    task automatic get_write_response_phase(axi4_transaction trans);
      bit [((AXI4_ID_WIDTH) - 1) : 0] trans_id;
      bit [((AXI4_USER_WIDTH) - 1):0] temp_resp_user;
