...
//
// questa_mvc_svapi.h
//   Definition of special types and API which may be used for interfacing
//   between SystemVerilog and Questa MVC.
//
//   Types:
//     questa_mvc_transaction_handle - handle for transaction objects
//     questa_mvc_external_db - handle used to refer to external databases
//     questa_mvc_iterator - handle for iterating over Questa MVC objects
//     questa_mvc_show_content_enum - enum for controlling the output of the 'show_item' task
//     questa_mvc_time_relation_enum - enum for controlling the time-relationship on items
//     questa_mvc_dont_care_enum - enum for describing 'don't-care' states
//     questa_mvc_timecale_enum - enum for describing time units
//     questa_mvc_item_disable_enum - enum for controlling the enabling/disabling of the ability of a Questa MVC item to do various actions
//     questa_mvc_time - class which represents time as a value,unit pair. Includes method to convert this absolute time to Questa MVC simulation time
//     questa_mvc_reporter - class which enables the user to control Questa MVC messages
//
//   API (multiple versions):
//     questa_mvc_find - for searching for objects by name
//     questa_mvc_valid_handle - checks whether an iterator refers to a valid object
//     questa_mvc_advance_handle - advances an iterator through questa_mvc_find results
//     questa_mvc_release_handle - releases a handle once used
//     questa_mvc_name_from_iterator - gets the name of an object from an iterator (handle)
//     questa_mvc_*_clock(s) - for controlling Questa MVC internal clocks
//     questa_mvc_hist_* - for opening, recording to and closing an external database
//     questa_mvc_show   - for outputing information about the Questa MVC model.
//     questa_mvc_do_cmd - Execute a command within the MVC
//     questa_mvc_set_time_relation(_pattern) - for controlling the behaviour of time relations
//     questa_mvc_set_dont_care(_pattern) - for controlling the bahaviour of don't care specifications
//     questa_mvc_sv_convert_to_precision(time_value,time_unit) - for converting an absolute time to Questa MVC simulation time
//
//   Interface:
//     questa_mvc_reporting_if - interface used to get Questa MVC messages into SystemVerilog
//
// --------------------------------------------------------------------
...
An enum for specifying time-units.
...
QUESTA_MVC_TIME_X    - Unspecified time unit (equates to the simulator time-step i.e. the smallest of the precision declarations in the design compilation units)
*/
typedef enum
{
...
An enum for specifying a required communication semantic. For more details of the communication semantics, please see <QuestaMVC Communication Semantics>.

QUESTA_MVC_COMMS_SENT                    - Sent
QUESTA_MVC_COMMS_SEND                    - Send
QUESTA_MVC_COMMS_SENDING                 - Sending
QUESTA_MVC_COMMS_SENT_TOP                - Sent that is always allowed - use with care as it does not check that it is legal protocol
QUESTA_MVC_COMMS_SEND_TOP                - Send that is always allowed - use with care as it does not check that it is legal protocol
QUESTA_MVC_COMMS_SENDING_TOP             - Sending that is always allowed - use with care as it does not check that it is legal protocol
QUESTA_MVC_COMMS_ACTIVATE                - Activate
QUESTA_MVC_COMMS_ACTIVATING              - Activating
QUESTA_MVC_COMMS_ACTIVATES               - Activates
QUESTA_MVC_COMMS_ACTIVATED               - Activated
QUESTA_MVC_COMMS_ACTIVATE_TOP            - Activate that is always allowed - use with care as it does not check that it is legal protocol
QUESTA_MVC_COMMS_ACTIVATING_TOP          - Activating that is always allowed - use with care as it does not check that it is legal protocol
QUESTA_MVC_COMMS_ACTIVATES_TOP           - Activates that is always allowed - use with care as it does not check that it is legal protocol
QUESTA_MVC_COMMS_ACTIVATED_TOP           - Activated that is always allowed - use with care as it does not check that it is legal protocol
QUESTA_MVC_COMMS_RECEIVE                 - Receive
QUESTA_MVC_COMMS_RECEIVED                - Received
QUESTA_MVC_COMMS_RECEIVING               - Receiving
QUESTA_MVC_COMMS_RECEIVE_CONSUMING       - Receive and this item will be consumed stopping it being received again
QUESTA_MVC_COMMS_RECEIVED_CONSUMING      - Received and this item will be consumed stopping it being received again
QUESTA_MVC_COMMS_RECEIVING_CONSUMING     - Receiving and this item will be consumed stopping it being received again
QUESTA_MVC_COMMS_RECEIVE_VIRTUAL         - Receive Virtual
QUESTA_MVC_COMMS_RECEIVE_TOP             - Receive causing an item to always be allowed - use with care as it does not check that it is legal protocol
QUESTA_MVC_COMMS_RECEIVED_TOP            - Received causing an item to always be allowed - use with care as it does not check that it is legal protocol
QUESTA_MVC_COMMS_RECEIVING_TOP           - Receiving causing an item to always be allowed - use with care as it does not check that it is legal protocol
QUESTA_MVC_COMMS_RECEIVE_TOP_CONSUMING   - Receive causing an item to always be allowed and this item will be consumed stopping it being received again - use with care as it does not check that it is legal protocol
QUESTA_MVC_COMMS_RECEIVED_TOP_CONSUMING  - Received causing an item to always be allowed and this item will be consumed stopping it being received again - use with care as it does not check that it is legal protocol
QUESTA_MVC_COMMS_RECEIVING_TOP_CONSUMING - Receiving causing an item to always be allowed and this item will be consumed stopping it being received again - use with care as it does not check that it is legal protocol
QUESTA_MVC_COMMS_SENDCONC                - Sendconc
*/
typedef enum
{
...
time_value - a 64-bit signed integer giving the number of time-units required
time_unit - an int (using one of the valid values from <questa_mvc_timecale_enum>), specifying the time-unit 
*/
