...
// $Id: //acds/main/ip/merlin/altera_merlin_axi_translator/altera_merlin_axi_translator.sv#10 $
// $Revision: #10 $
// $Date: 2012/08/06 $
// $Author: tgngo $
// -----------------------------------------
...
module altera_merlin_axi_bridge
#( 
    // ----------------
...
(
    ...
    output [S0_ADDR_WIDTH-1:0]                     m0_awaddr,
    ...
    output [S0_ADDR_WIDTH-1:0]                     m0_araddr,
    ...
    localparam AX_WIDTH         =   (M0_AXI_VERSION == "AXI3")? 
                                        M0_ID_WIDTH+S0_ADDR_WIDTH+M0_BURST_LENGTH_WIDTH+3+2+M0_LOCK_WIDTH+4+3+M0_WRITE_ADDR_USER_WIDTH : 
                                        M0_ID_WIDTH+S0_ADDR_WIDTH+M0_BURST_LENGTH_WIDTH+3+2+M0_LOCK_WIDTH+4+3+M0_WRITE_ADDR_USER_WIDTH+4+4 ;
    localparam W_WIDTH          =   (M0_AXI_VERSION == "AXI3")?
                                        M0_ID_WIDTH+DATA_WIDTH+S0_ADDR_WIDTH+STROBE_WIDTH+1 :
                                        M0_ID_WIDTH+DATA_WIDTH+S0_ADDR_WIDTH+STROBE_WIDTH+1+M0_WRITE_DATA_USER_WIDTH ;

    localparam B_WIDTH          =   (M0_AXI_VERSION == "AXI3")?
                                        M0_ID_WIDTH+2 : 
                                        M0_ID_WIDTH+2+M0_WRITE_DATA_USER_WIDTH;

    localparam R_WIDTH          =   (M0_AXI_VERSION == "AXI3")?
                                        M0_ID_WIDTH+DATA_WIDTH+2+1 :
                                        M0_ID_WIDTH+DATA_WIDTH+2+1+M0_READ_DATA_USER_WIDTH;
    ...
    localparam PKT_AXLOCK_H     = PKT_AXLOCK_L + M0_LOCK_WIDTH - 1;
    ...
    localparam PKT_AXLEN_H      = PKT_AXLEN_L + M0_BURST_LENGTH_WIDTH -1;
    localparam PKT_AXADDR_L     = PKT_AXLEN_H + 1;
    localparam PKT_AXADDR_H     = PKT_AXADDR_L + S0_ADDR_WIDTH -1;
    localparam PKT_AXID_L       = PKT_AXADDR_H + 1;
    localparam PKT_AXID_H       = PKT_AXID_L + M0_ID_WIDTH -1;
    localparam PKT_AXUSER_L     = PKT_AXID_H + 1;
    localparam PKT_AXUSER_H     = PKT_AXUSER_L + M0_WRITE_ADDR_USER_WIDTH -1;
    ...
    localparam PKT_WUSER_H      = PKT_WUSER_L + M0_WRITE_DATA_USER_WIDTH -1;
    ...
    localparam PKT_BUSER_H      = PKT_BUSER_L + M0_WRITE_DATA_USER_WIDTH -1;
    ...
    localparam PKT_RUSER_H      = PKT_RUSER_L + M0_READ_DATA_USER_WIDTH -1;   

    reg [M0_ID_WIDTH-1:0]                       s0_pipe_awid;
    reg [S0_ADDR_WIDTH-1:0]                     s0_pipe_awaddr;
    reg [M0_BURST_LENGTH_WIDTH-1:0]             s0_pipe_awlen;
    reg [2:0]                                   s0_pipe_awsize;
    reg [1:0]                                   s0_pipe_awburst;
    reg [M0_LOCK_WIDTH-1:0]                     s0_pipe_awlock;
    reg [3:0]                                   s0_pipe_awcache;
    reg [2:0]                                   s0_pipe_awprot;
    reg [M0_WRITE_ADDR_USER_WIDTH-1:0]          s0_pipe_awuser;
    ...
    reg [S0_ADDR_WIDTH-1:0]                     s0_pipe_araddr;
    reg [M0_BURST_LENGTH_WIDTH-1:0]             s0_pipe_arlen;
    reg [2:0]                                   s0_pipe_arsize;
    reg [1:0]                                   s0_pipe_arburst;
    reg [M0_LOCK_WIDTH-1:0]                     s0_pipe_arlock;
    reg [3:0]                                   s0_pipe_arcache;
    reg [2:0]                                   s0_pipe_arprot;
    reg [M0_WRITE_ADDR_USER_WIDTH-1:0]          s0_pipe_aruser;
    ...
    reg [M0_WRITE_DATA_USER_WIDTH-1:0]          s0_pipe_wuser;
    reg                                         s0_pipeout_wvalid;
    reg                                         s0_pipe_wvalid;

    reg [M0_ID_WIDTH-1:0]                        m0_pipe_bid;
    reg [1:0]                                    m0_pipe_bresp;
    reg [M0_WRITE_RESPONSE_DATA_USER_WIDTH-1:0]  m0_pipe_buser;
    ...
    reg                                          m0_pipe_rlast;
    reg [M0_WRITE_RESPONSE_DATA_USER_WIDTH-1:0]  m0_pipe_ruser;
...
//======================================================================
    ...
    begin
        if (M0_AXI_VERSION == "AXI3") begin
            ...
            s0_pipe_awuser = s0_awuser[S0_WRITE_ADDR_USER_WIDTH-1:0];
        end else begin
            if (!USE_S0_AWREGION)
                s0_pipe_awregion = '0;
            else
    ...
    generate if (M0_AXI_VERSION == "AXI3") begin
    ...
    generate if (M0_AXI_VERSION == "AXI4") begin
    ...
    begin
        if (M0_AXI_VERSION == "AXI3") begin
            ...
            if (!USE_S0_WUSER)
                ...
                s0_pipe_wuser = s0_wuser[S0_WRITE_DATA_USER_WIDTH-1:0];   
            // non-optional signals for slave-side interface -propagate these to master-side interface
            s0_pipe_wid = s0_wid[M0_ID_WIDTH-1:0];
            s0_pipe_wdata = s0_wdata;
            s0_pipe_wstrb = s0_wdata;    
        end
    end
    ...
    begin
        if (M0_AXI_VERSION == "AXI3") begin
            ...
            if (!USE_S0_ARUSER)
                ...
                s0_pipe_aruser = s0_aruser[S0_READ_ADDR_USER_WIDTH-1:0];
