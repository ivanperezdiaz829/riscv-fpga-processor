...
proc sanity_checks { speed device core  } {
    ...
    if { ($core == "MT64" || $core == "T64") && ($speed == "66") && ($pin_count == "672") } {
            puts "Error: Altera does not support the pci_mt64 and pci_t64 function in F672 packages for 66 MHz PCI"
            puts "       Change the Arria GX device package in your project"
            error ""
    }

	if { ($core == "MT64" || $core == "T64") && ($pin_count == "484") } {
            puts "Error: Altera does not support the pci_mt64 and pci_t64 function in F484 packages for PCI"
            puts "       Change the Arria GX device package in your project"
            error ""
    }
