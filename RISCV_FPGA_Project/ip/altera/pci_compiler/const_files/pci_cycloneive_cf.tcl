# Cyclone IV E Constraint file.

# PCI_CF_Device_Family = Cyclone IV E
...
proc sanity_checks { speed device core  } {
        ...
        # Check if customer is targeting on Cyclone IV E device
	set device_family_name "Cyclone IV E"
        set device_family [lindex [get_part_info -family $device] 0] 
        if { $device_family == $device_family_name } {
        } else {
                puts "Error: This constraint file is specific to $device_family_name device family."
                puts "       You are not targeting a $device_family_name device."
        ...
        if { ($core == "MT64" || $core == "T64") && ($pin_count == "148" || $pin_count == "169") } {
                puts "Error: Altera does not support the pci_mt64 and pci_t64 function in F169 and Q148 packages"
                puts "       Change the $device_family_name device package in your project"
                error ""
        }       
        
		if { ($core == "MT64" || $core == "T64") && ($pin_count == "144") } {
                puts "Error: Altera does not support the pci_mt64 and pci_t64 function in E22 packages"
                puts "       Change the $device_family_name device package in your project"
                error ""
        }
}
...
proc pci_pin_asgn {device full_64 master core pin_list no_pins got_prefix got_suffix prefix suffix use_dedicated_clk } {
        ...
        # Container constraints for Cyclone IV E
...
puts "Info: You have sourced the Cyclone IV E PCI constraint file."
