...
proc sanity_checks { speed device core  } {
	...
	pci_messages $device 1 
		...
		if { $speed == "66" && $grade == "4L" } {
			pci_messages $device 0
		} elseif { $device_type == "EP3SL340" } {
			if { $grade == "4" } {
				pci_messages $device 0
			} else {
				pci_messages $device 1
			}
		} elseif { $device_type == "EP3SL200" || $device_type == "EP3SE260" } { 
		if {$tmp_grade == "Industrial" && $grade == "4" && ($core == "T32" || $core == "T64") } {
			pci_messages $device 0
        	} else {
                	pci_messages $device 1
        	}
	} elseif { $device_type == "EP3SE50" || $device_type == "EP3SE80" || $device_type == "EP3SE110" || $device_type == "EP3SL50" || $device_type == "EP3SL70" || $device_type == "EP3SL110" || $device_type == "EP3SL150"  } {
		pci_messages $device 1
	} else {
