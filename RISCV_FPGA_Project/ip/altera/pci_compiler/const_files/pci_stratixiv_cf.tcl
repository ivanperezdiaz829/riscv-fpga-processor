...
proc add_pci_constraints {args} {
	...
	set prefix "0" 
	set suffix "0" 
	global CRITICAL_WARNING
	set CRITICAL_WARNING "0"
...
proc run_pin_names_check { pin_list } {
	...
	set pci_io [get_all_instance_assignments -name CLAMPING_DIODE]
	
	# Defaults pin names to "match"
	set match 1
	
	#{ {} {} {ack64n} {CLAMPING_DIODE} {ON}}
...
proc family_pci_asgn { master full_64 hierarchy pin_list got_prefix got_suffix prefix suffix } {
	...
	# For QIC flow, SPR 316344 
       	set_instance_assignment -name FAST_OUTPUT_REGISTER -to "*trdy_OR_NOT" ON
	...
	if {$master} {  
		# For QIC flow, SPR 316344 
       		set_instance_assignment -name FAST_OUTPUT_REGISTER -to "*irdy_or_not" ON
...
proc sanity_checks { speed device core  } {
	...
	if {$speed == "33" } {
		...
		# PCI 66MHz Support
		if {$grade == "4" } {
			# All C4/I4 does not support 66MHz
			pci_messages $device 0 
		} elseif { $device_type == "EP4SGX70" || $device_type == "EP4SGX110" || $device_type == "EP4SE110" } {
			pci_messages $device_type 1 
		} elseif { $device_type == "EP4SE360" } {
			if { $grade == "3" && ($core =="T32" || $core =="T64") } {
                		pci_messages $device 0
	                } else {
				pci_messages $device 1
			}
		} elseif { $device_type == "EP4SGX180" || $device_type == "EP4SE230" || $device_type == "EP4SGX230" || $device_type == "EP4SE530" || $device_type == "EP4SE820" || $device_type == "EP4SGX530" } {
			if { $grade == "3" } {
				pci_messages $device 0
			} else {
				pci_messages $device 1
			}
		} elseif { $device_type == "EP4SGX290N" || $device_type == "EP4SGX360N" } {
			if { $pin_count == "1932" && $grade == "3" } {
				# NF45 does not support C3/I3 at 66MHz
				pci_messages $device 0
			} else {
				pci_messages $device 1
			}
		} elseif { $device_type == "EP4SGX290" || $device_type == "EP4SGX360" } {
			if { $pin_count == "1517" && $grade == "3" && ($core =="MT64" || $core == "MT32")} {
				pci_messages $device 1
			} elseif { $pin_count != "1932" && $grade == "3" && ($core =="T32" || $core =="T64")} {
				pci_messages $device 0
                        } else {
                                pci_messages $device 1
                        }
		} elseif { $device_type == "EP4S40G2" || $device_type == "EP4S40G5" || $device_type == "EP4S100G2" || $device_type == "EP4S100G3" || $device_type == "EP4S100G4" || $device_type == "EP4S100G5" } {
			if { $grade == "3" } {
				pci_messages $device 0
                        } else {
                                pci_messages $device 1
...
proc pci_messages { device support } {
   ...
   if { $support == "1" } {
	...
	puts "PCI Critical Warning: PCI timing requirements are not met in device $device.  "
	set CRITICAL_WARNING 1
   }
}

#**********************
# Check if any PCI Critical Warnings 
#**********************
proc check_critical_warnings { } {
   global CRITICAL_WARNING 
   if { $CRITICAL_WARNING != "0" } {
        puts "PCI Critical Warning: Device may fail to compile successfully. "
		error ""
   } 
}
