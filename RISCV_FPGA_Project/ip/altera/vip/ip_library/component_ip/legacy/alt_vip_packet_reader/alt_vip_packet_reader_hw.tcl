...
# --------------------------------------------------------------------------------------------------
# --                                                                                              --
# -- General information for the packet reader component                                          --
# -- This block receives packet read commands (on the cmd interface), reads the packet            --
# -- information on the Avalon-MM master interface and then sources it on the Avalon-ST source    --
# --                                                                                              --
# --------------------------------------------------------------------------------------------------
add_file alt_vip_packet_reader.sdc SDC
...
set_module_property   NAME alt_vip_packet_reader
set_module_property   DISPLAY_NAME   "Packet Reader"
set_module_property   DESCRIPTION    "Receives packet read commands (on the cmd interface), reads the packet data on the read_master Avalon-MM master interface and sources it on the dout Avalon-ST streaming interface"
set_module_property   TOP_LEVEL_HDL_FILE src_hdl/alt_vip_packet_reader.sv
set_module_property   TOP_LEVEL_HDL_MODULE alt_vip_packet_reader

set_module_property ELABORATION_CALLBACK packet_reader_elaboration_callback
set_module_property VALIDATION_CALLBACK packet_reader_validation_callback
...
add_file src_hdl/alt_vip_timing_adapter_fifo.sv $add_file_attribute
add_file src_hdl/alt_vip_timing_adapter.sv $add_file_attribute
add_file src_hdl/alt_vip_burst_calculator.sv $add_file_attribute
add_file src_hdl/alt_vip_packet_reader.sv $add_file_attribute
...
add_parameter REGISTER_OUTPUT int 1
set_parameter_property   REGISTER_OUTPUT   AFFECTS_GENERATION              true
...
set_parameter_property   PIPELINE_READY    AFFECTS_GENERATION              true
...
add_parameter DOUT_SRC_ADDR int 0
set_parameter_property   DOUT_SRC_ADDR     AFFECTS_GENERATION              true
set_parameter_property   DOUT_SRC_ADDR     HDL_PARAMETER                   true
set_parameter_property   DOUT_SRC_ADDR     DISPLAY_NAME                    "Source ID of packet reader on dout interface"
...
# --                                                                                              --
# --------------------------------------------------------------------------------------------------
proc packet_reader_elaboration_callback {} {
    ...
    set src_id                      [get_parameter_value DOUT_SRC_ADDR]
    ...
    if { $separate_clocks > 0 } {
       ...
       add_bursting_read_master_port   av_mm_read_master   $mem_port_width     $burst_target      $burst_align          av_mm_clock
    } else {
       add_bursting_read_master_port   av_mm_read_master   $mem_port_width     $burst_target      $burst_align          main_clock
    }
...
# --                                                                                              --
# --------------------------------------------------------------------------------------------------
proc packet_reader_validation_callback {} {
   ...
   if { $st_data_width > $mem_port_width } {
      send_message error "Data with for av_st_dout (currently $st_data_width) must not be grater than data width for av_mm_read_master (currently $mem_port_width)"
   }
